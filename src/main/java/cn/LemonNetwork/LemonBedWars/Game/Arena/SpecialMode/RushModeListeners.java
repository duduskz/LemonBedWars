        ...
        try {
            if (event.getItem().getType().equals(Material.WOOL)) {
                if (event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() == Action.LEFT_CLICK_AIR) {
                    if (LemonBedWars.onSpeed.get(event.getPlayer().getName())) {
                        ActionBar.sendMessage(event.getPlayer(), "§c§l搭路模式已关闭");
                        LemonBedWars.onSpeed.replace(event.getPlayer().getName(), false);
                    } else {
                        ActionBar.sendMessage(event.getPlayer(), "§a§l搭路模式已开启");
                        LemonBedWars.onSpeed.replace(event.getPlayer().getName(), true);
                    }
                }
            }
    ...
    public void onBlockPlace(final BlockPlaceEvent event) {
        if (LemonBedWars.onSpeed.get(event.getPlayer().getName())) {
            ...
            if (!event.isCancelled())
                (new BukkitRunnable() {
                    ...
                    public void run() {
                        if (this.place <= 4 && nextBlock
                            ...
                            LemonBedWars.changedBlocks.add(nextBlock.getBlock().getLocation());
                ...
                }).runTaskTimer(LemonBedWars.getPlugin(LemonBedWars.class), 2L, 3L);
        }
    }
}